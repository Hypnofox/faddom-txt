...
**Click:** Save this search as a map

**Say:**
"And here's the magic - one click, this search becomes a permanent map that tracks changes automatically. 

You don't build maps manually. You don't maintain them. You don't update them.

You search once, save it, and Faddom keeps it current forever. When servers are added or removed, when connections change, the map updates itself."

**Value statement:**
"This replaces tribal knowledge on day one. The engineer who set this up five years ago and just left? You don't need them anymore. And creating documentation just became a 10-second task instead of a 10-hour project."

**DROP THE MIC MOMENT #1:** Search → Save → Live map that maintains itself forever
...
**Switch to Tier View:**
"Now watch this - I can collapse this into logical tiers. Instead of seeing 50 individual servers, you see: web tier, app tier, database tier. This is how executives and application owners want to see their applications - by function, not by server name."

**Show Tags:**
"And here's something powerful - Faddom automatically imports tags from VMware, AWS, Azure, GCP, Nutanix. Cost center tags, environment tags, owner tags - everything.

You can build maps directly from tags. For example: Show me everything tagged 'Production-Finance'. One click, automatic map, always current.

The only platform that doesn't export tags automatically is Hyper-V - for those, you can import them manually or via API."

**Value from tags:**
"This means your existing organizational structure - the tags your finance team uses for chargeback, the tags your cloud team uses for governance - they instantly become powerful map filters in Faddom. No duplicate tagging, no manual work."
...
## SECTION 11: USER DISCOVERY & AD INTEGRATION (6 minutes)
### Goal: Show who depends on what and detect security anomalies

**Click:** Inventory → Users

**Say:**
"This is one of the most underrated features. Through Active Directory integration, Faddom shows you which users logged into which servers and when.

But more importantly - we map users to the applications they actually depend on."

**Click on a user → Open user map**

**Say:**
"This shows exactly which servers and applications this user relies on to do their job.

Why does this matter?

First: When you take a server down for maintenance, you know exactly which users will be affected. No more surprise calls from the CEO saying 'I can't access my reports.'

Second: Security. See this user who hasn't logged in for 90 days? That's a dormant account that should probably be disabled.

See this user logging in from multiple servers at 3 AM? That might be legitimate admin work... or it might be a compromised account.

Third: Offboarding. When someone leaves the company, you see exactly which systems they had access to - not what they were supposed to have access to according to AD groups, but what they actually used."

**Show the recently logged-in users view:**

**Say:**
"And here's a real example from last month: A customer discovered a user account that had left the company 6 months ago, still actively logging into servers. Faddom caught the trace. That was a breach waiting to happen."

**Value statement:**
"You can see exactly who will be affected when a server goes offline - bringing it full circle to the 'no more scream test' principle. Be proactive, not reactive."

**CTO might ask:** "What permissions does this require?"
**Answer:** "WMI access for Windows servers, SSH for Linux. We read Windows event logs for login history - same permissions your SIEM or monitoring tools use. Can run through a proxy if needed for security segmentation. Nothing is installed on the endpoints themselves."

---

## SECTION 12: TECHNICAL ARCHITECTURE (5 minutes - Only if CTO asks)
